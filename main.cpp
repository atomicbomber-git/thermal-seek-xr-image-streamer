...
void draw_temp(Mat &outframe, double temp, const Point& coord, Scalar color) {
    ...
    putText(outframe, txt, coord-Point(20, -20), FONT_HERSHEY_COMPLEX_SMALL, 0.75, std::move(color), 1, CV_AA);
}
...
int main(int argc, char **argv) {
    ...
    sf::TcpSocket socket;
    const auto remoteAddress = args::get(arg_target_host).c_str();
    const auto remotePort = std::stoi(args::get(arg_target_port));
    ...
    while (!sigflag) {
        ...
        putText(outFrame, getTime().str(), Point(10, 20), FONT_HERSHEY_COMPLEX_SMALL, 0.75, Scalar(0,0, 0), 1, CV_AA);
        ...
        if (socket.send(imageSizeText, strlen(imageSizeText)) == sf::Socket::Done) {
        } else {
            connectToServer(socket, remoteAddress, remotePort);
        }

        if (socket.send(&imageBuffer[0], imageSize) == sf::Socket::Done) {
        } else {
            connectToServer(socket, remoteAddress, remotePort);
        };
    }
